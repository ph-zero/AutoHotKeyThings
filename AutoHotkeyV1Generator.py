...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QGridLayout, QMessageBox, QFrame, QSplitter, QListWidget, QListWidgetItem)
...
class AHKGeneratorApp(QMainWindow):
    ...
    def initUI(self):
        ...
        for var, desc in dynamic_vars:
            ...
            dynamic_vars_list.addItem(item)
        
        dynamic_vars_list.itemDoubleClicked.connect(self.insert_dynamic_variable)
        side_layout.addWidget(dynamic_vars_list)
        ...
        usage_example = QLabel(
            "To insert the current date, use:\n"
            "Today is {A_YYYY}-{A_MM}-{A_DD}\n\n"
            "This will output:\n"
            "Today is 2023-10-15"
        )
        usage_example.setWordWrap(True)
        usage_example.setStyleSheet("""
            background-color: rgba(0, 0, 0, 0.2);
            padding: 10px;
            border-radius: 5px;
            font-size: 13px;
        """)
        side_layout.addWidget(usage_example)
    ...
    def generate_script(self):
        script_lines = [
            "; AutoHotkey Script Generated by Python UI",
        ...
        for i, input_field in enumerate(self.input_fields):
            ...
            if text:
                ...
                text = text.replace("{Backspace}", "{Backspace}")
                
                script_lines.extend([
                    f"+F{i+1}::",
                    f"    FormatTime, CurrentDateTime,, yyyy-MM-dd HH:mm:ss",
                    f"    Send, {text}",
                    "    Return",
                    ""
                ])
